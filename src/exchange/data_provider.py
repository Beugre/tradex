...
Couche d'abstraction au-dessus de RevolutXClient pour fournir
...
# Intervalle H4 en minutes (l'API Revolut X attend des minutes)
H4_INTERVAL_MINUTES = 240  # 4 heures


class DataProvider:
    """Fournit les données de marché depuis l'API Revolut X."""
    ...
    def get_ticker(self, symbol: str) -> Optional[TickerData]:
        ...
        for t in tickers:
            if t.symbol.replace("/", "-") == symbol or t.symbol == symbol:
                return t
