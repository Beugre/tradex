...
class LondonBreakoutBot:
    ...
    def __init__(self, dry_run: bool = False) -> None:
        ...
        # Paper mode
        self._paper_mode = config.is_paper("london")
        ...
        if self._paper_mode:
            self._client = PaperRevolutClient(
                self._client, initial_balance=config.PAPER_BALANCE_LONDON,
                bot_name="london",
            )
            from src.firebase.trade_logger import set_paper_mode
            set_paper_mode(True)
            logger.info("📄 [LONDON] Mode PAPER TRADING activé — balance $%.0f", config.PAPER_BALANCE_LONDON)
