"""
TradeX Binance Dashboard — Monitoring temps réel du bot Binance.
Lit les données depuis Firebase Firestore avec filtre exchange="binance".
Port: 8503.
"""
...
st.set_page_config(
    page_title="TradeX Binance Dashboard",
    page_icon="🟡",
    layout="wide",
...
def _fetch_trades(days: int = 90) -> pd.DataFrame:
    """Récupère les trades Binance depuis Firestore."""
    db = _get_db()
...
def _fetch_daily_snapshots(days: int = 90) -> pd.DataFrame:
    """Récupère les snapshots journaliers Binance."""
    db = _get_db()
...
def _fetch_open_positions() -> pd.DataFrame:
    """Récupère les trades Binance ouverts."""
    db = _get_db()
    docs = (
        db.collection("trades")
        .where("exchange", "==", EXCHANGE)
...
def _fetch_close_failures(hours: int = 24) -> pd.DataFrame:
    """Récupère les événements CLOSE_FAILURE Binance récents."""
    db = _get_db()
...
st.sidebar.title("TradeX · Binance")
st.sidebar.caption("Bot USDC — Range Only + OCO natifs")
...
st.title("🟡 TradeX Binance Dashboard")
...
if not open_df.empty:
    ...
    st.info("Aucune position Binance ouverte")
...
if not snapshots_df.empty and "equity" in snapshots_df.columns:
    fig_eq = px.area(
        ...
        color_discrete_sequence=["#f0b90b"],  # Binance yellow
    )
...
if not closed.empty:
    ...
    st.info("Aucun trade Binance fermé sur cette période")
...
st.divider()
st.caption(
    f"TradeX Binance Dashboard • Données Firebase (exchange=binance) • "
