"""
TradeX Binance BREAKOUT Dashboard — Monitoring du bot Breakout Volatility Expansion.
Lit les données depuis Firebase Firestore avec filtre exchange="binance-breakout".
Port: 8504.
"""
...
EXCHANGE = "binance-breakout"

st.set_page_config(
    page_title="TradeX Breakout Dashboard",
    page_icon="🔥",
    layout="wide",
...
def _fetch_trades(days: int = 90) -> pd.DataFrame:
    """Récupère les trades Breakout depuis Firestore."""
    db = _get_db()
...
def _fetch_daily_snapshots(days: int = 90) -> pd.DataFrame:
    """Récupère les snapshots journaliers Breakout."""
    db = _get_db()
...
def _fetch_open_positions() -> pd.DataFrame:
    """Récupère les trades Breakout ouverts."""
    db = _get_db()
    docs = (
        db.collection("trades")
        .where("exchange", "==", EXCHANGE)
...
def _fetch_events(event_type: str | None = None, hours: int = 48) -> pd.DataFrame:
    """Récupère les événements Breakout récents."""
    db = _get_db()
...
st.sidebar.image("https://img.icons8.com/color/96/fire-element.png", width=60)
st.sidebar.title("TradeX · Breakout")
st.sidebar.caption("Bot USDC — Breakout Volatility Expansion · Long Only")
...
st.title("🔥 TradeX Breakout Dashboard")
...
# ── Positions ouvertes (Breakout — avec trailing info) ─────────────────────────
...
if not open_df.empty:
    ...
    rename_map = {
        ...
        "sl_price": "Trail SL", "peak_price": "Peak",
    ...
    st.info("Aucune position Breakout ouverte")


# ── Kill-switch status ─────────────────────────────────────────────────────────

kill_events = _fetch_events("KILL_SWITCH", hours=720)  # 30 jours
...
if not snapshots_df.empty and "equity" in snapshots_df.columns:
    fig_eq = px.area(
        ...
        color_discrete_sequence=["#ff6d00"],  # Orange breakout
    )
...
# ── Trailing stats (gain peak vs entry) ───────────────────────────────────────
...
if not closed.empty:
    ...
    st.info("Aucun trade Breakout fermé sur cette période")
...
st.divider()
st.caption(
    f"TradeX Breakout Dashboard • Données Firebase (exchange={EXCHANGE}) • "
    f"Stratégie: Breakout Volatility Expansion — Long Only + Adaptive Trail + Kill-Switch • "
